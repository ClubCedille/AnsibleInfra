...
| 66              | 10.66.0.0/16                     | CLIENT (réseau joueurs CTF)             | 🟠     | *(event OPNsense supprimé)* — **réservé événements futurs**                        | 0 VM active — teardown SummerCamp 2026-06-23 ; **à conserver sur le trunk switch**                                                                   |
| 67              | 10.67.0.0/16                     | AP-MGMT (WiFi + mgmt event)             | 🟠     | *(event OPNsense supprimé)* — **réservé événements futurs**                        | 0 VM active — **à conserver sur le trunk switch**                                                                                                    |
| 68              | 10.68.0.0/16                     | DNS/Challenges CTF                      | 🟠     | *(event OPNsense supprimé)* — **réservé événements futurs**                        | 0 VM active — **à conserver sur le trunk switch**                                                                                                    |
| 69              | 10.69.0.0/16                     | Réseau événement additionnel            | 🟠     | *(event OPNsense supprimé)* — **réservé événements futurs**                        | 0 VM active — **à conserver sur le trunk switch**                                                                                                    |
...
- **Correction 2026-09-17** : les réseaux des VLANs 66-69 ont été corrigés vers
  `10.66-69.0.0/16` (10.\<VLAN\>.0.0/16), l'ancien `10.110/120/130/140.0.0/16` étant
  obsolète (cf. `CLAUDE.md`). L'infra événementielle est actuellement détruite —
  adressage à utiliser à la prochaine saison, `playbooks/sc/vars/flags.yml` et les
  group_vars event/summercamp restent à réaligner avant tout redéploiement.
